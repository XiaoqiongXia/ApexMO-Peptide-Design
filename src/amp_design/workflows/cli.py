...
from amp_design.workflows.config import (
    OptimizationConfig,
    PipelineConfig,
    PublicationConfig,
    SamplingConfig,
...
def check_optimization(config: OptimizationConfig) -> dict[str, object]:
    from amp_design.predictors.apex import discover_models, load_strain_groups
    from amp_design.predictors.apex_pathogen import discover_pathogen_models
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command in publication_commands:
        from amp_design.workflows.publication import (
    ...
    if args.command in {
        ...
        from amp_design.workflows.release import (
        ...
        optimization = OptimizationConfig.from_file(args.config)
        from amp_design.optimization.search import run_optimization
        ...
        if PublicationConfig.is_configured(args.config):
            from amp_design.workflows.publication import run_publication_pipeline
