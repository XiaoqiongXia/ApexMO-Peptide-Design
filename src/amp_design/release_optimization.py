...
from .activity_scoring import aggregate_activity_targets, score_activity_command
from .apex import discover_models, load_strain_groups, score_apex_ensemble
from .apex_pathogen import discover_pathogen_models, score_apex_pathogen_ensemble
...
from .release_config import OptimizationConfig
...
def frozen_hashes(config: OptimizationConfig) -> dict[str, str]:
    ...
    if config.activity_scorer == "apex":
        activity_paths = [
            Path(__file__).with_name("apex.py").resolve(),
        ...
        activity_paths = [
            Path(__file__).with_name("apex_pathogen.py").resolve(),
            config.apex_root / "APEX_models.py",
            *discover_pathogen_models(config.apex_root),
        ]
    else:
        activity_paths = [
            Path(__file__).with_name("activity_scoring.py").resolve(),
            *config.activity_model_files,
        ]
    paths = [
        Path(__file__).resolve(),
        Path(__file__).with_name("activity_scoring.py").resolve(),
        Path(__file__).with_name("genetic_pareto.py").resolve(),
        Path(__file__).with_name("release_config.py").resolve(),
