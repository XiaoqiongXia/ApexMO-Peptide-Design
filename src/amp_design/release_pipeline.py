...
from .pareto_finalization import apply_cluster_cap, finalize_combined_archive
from .release_config import OptimizationConfig, PipelineConfig
from .release_optimization import (
    ObjectiveEvaluator,
    _atomic_csv,
    _atomic_parquet,
    _atomic_text,
...
def finalize_optimization(
    ...
    metadata = {
        ...
        "code_sha256": {
            ...
            str(Path(__file__).with_name("pareto_finalization.py").resolve()): (
                _sha256_file(Path(__file__).with_name("pareto_finalization.py"))
            ),
        },
