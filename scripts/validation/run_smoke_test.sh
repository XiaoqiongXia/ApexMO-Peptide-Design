...
if [[ ! -f "${env_file}" ]]; then
  printf 'Missing %s; run bash scripts/setup/bootstrap.sh first.\n' "${env_file}" >&2
  exit 1
fi
...
sampling_smoke="${repo_root}/scripts/validation/check_sampling_inference.py"
if [[ ! -f "${sampling_smoke}" ]]; then
  sampling_smoke="${repo_root}/github_release_template/scripts/validation/check_sampling_inference.py"
fi
conda run --no-capture-output --name "${AMP_DESIGN_ENV_NAME}" \
  python "${sampling_smoke}" --config "${config}"

check_script="${repo_root}/scripts/validation/check_release_installation.py"
if [[ ! -f "${check_script}" ]]; then
  check_script="${repo_root}/github_release_template/scripts/validation/check_release_installation.py"
fi
conda run --no-capture-output --name "${AMP_DESIGN_ENV_NAME}" \
  env AMP_TOXINPRED3_PYTHON="${AMP_TOXINPRED3_PYTHON}" \
...
external_smoke="${repo_root}/scripts/validation/check_external_safety_installation.py"
if [[ ! -f "${external_smoke}" ]]; then
  external_smoke="${repo_root}/github_release_template/scripts/validation/check_external_safety_installation.py"
fi
conda run --no-capture-output --name "${AMP_DESIGN_ENV_NAME}" \
  env AMP_TOXINPRED3_PYTHON="${AMP_TOXINPRED3_PYTHON}" \
